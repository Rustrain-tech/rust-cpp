...
impl Graph {
    ...
    /// directed graph
    pub fn from_directed_edges(n: usize, edges: &Vec<(usize, usize)>) -> Self {
        let mut graph = Graph::new(n);
        for &(from, to) in edges {
            graph.add_edge(from, to, 1);
        }
    ...
    /// non-directed-weight graph
    pub fn from_weightedges(n: usize, edges: &Vec<(usize, usize, i64)>) -> Self {
        let mut graph = Graph::new(n);
    ...
    pub fn from_directed_weightedges(n: usize, edges: &Vec<(usize, usize, i64)>) -> Self {
        let mut graph = Graph::new(n);
        for &(from, to, cost) in edges {
            graph.add_edge(from, to, cost);
        }
